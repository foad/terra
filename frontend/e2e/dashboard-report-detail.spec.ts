...
test("click marker, detail panel opens, modal shows report", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await page.waitForFunction(() => {
      const map = (globalThis as { __MAP__?: { isStyleLoaded: () => boolean; queryRenderedFeatures: (opts: { layers: string[] }) => unknown[] } }).__MAP__;
      return map?.isStyleLoaded() && map.queryRenderedFeatures({ layers: ["report-markers"] }).length > 0;
    }, { timeout: 10000 });

    const point = await page.evaluate(
      ([lng, lat]) => {
        const map = (globalThis as { __MAP__?: { project: (c: [number, number]) => { x: number; y: number } } }).__MAP__;
